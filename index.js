...
const ownerNumber = ['918129540560']
...
if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
  ...
  const sessdata = config.SESSION_ID.replace("Zroxy~", '')
    ...
    conn.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        try {
          ...
          let up = `*Hᴇʟʟᴏ ᴛʜᴇʀᴇ ZROXY-MD ᴄᴏɴɴᴇᴄᴛᴇᴅ! 👋🏻* 
...
conn?.ev?.on('messages.update', async updates => {
  ...
  const udp = botNumber.split('@')[0];
    const malvin = ('918120540560','918606727281');
...
if (senderNumber.includes("918606727281") && !isReact) {
